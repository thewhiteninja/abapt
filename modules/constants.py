...
class WinTrustUIChoice(enum.Enum):
    WTD_UI_ALL = 1
    WTD_UI_NONE = 2
    WTD_UI_NOBAD = 3
    WTD_UI_NOGOOD = 4
...
class WinTrustRevocationChecks(enum.Enum):
    WTD_REVOKE_NONE = 0x00000000
    WTD_REVOKE_WHOLECHAIN = 0x00000001


class WinTrustUnionChoice(enum.Enum):
    WTD_CHOICE_FILE = 1
...
class WinTrustStateAction(enum.Enum):
...
class AjustPrivilege(enum.Enum):
...
class Token(enum.Enum):
    ...
    TOKEN_READ = STANDARD_RIGHTS_READ | 8
    TOKEN_WRITE = STANDARD_RIGHTS_WRITE | 32 | 64 | 128
...
class ProcessInformationQuery(enum.Enum):
    PROCESS_BASIC_INFORMATION = 0
    PROCESS_QUOTA_LIMITS = 1
    PROCESS_IO_COUNTERS = 2
    PROCESS_VM_COUNTERS = 3
    PROCESS_TIMES = 4
    PROCESS_BASE_PRIORITY = 5
    PROCESS_RAISE_PRIORITY = 6
    PROCESS_DEBUG_PORT = 7
    PROCESS_EXCEPTION_PORT = 8
    PROCESS_ACCESS_TOKEN = 9
    PROCESS_LDT_INFORMATION = 10
    PROCESS_LDT_SIZE = 11
    PROCESS_DEFAULT_HARD_ERROR_MODE = 12
    PROCESS_IO_PORT_HANDLERS = 13
    PROCESS_POOLED_USAGE_AND_LIMITS = 14
    PROCESS_WORKING_SET_WATCH = 15
    PROCESS_USERMODE_IOPL = 16
    PROCESS_ENABLE_ALIGNMENT_FAULT_FIXUP = 17
    PROCESS_PRIORITY_CLASS = 18
    PROCESS_WX86_INFORMATION = 19
    PROCESS_HANDLE_COUNT = 20
    PROCESS_AFFINITY_MASK = 21
    PROCESS_PRIORITY_BOOST = 22
    PROCESS_WOW64_INFORMATION = 26
    PROCESS_IMAGE_FILENAME = 27
    PROCESS_DEBUG_OBJECT_HANDLE = 30
    PROCESS_EXECUTE_FLAGS = 34
...
class FreeType(enum.Enum):
    MEM_DECOMMIT = 0x4000
    MEM_RELEASE = 0x8000


class DebugEvent(enum.Enum):
...
class ContinueStatus(enum.Enum):
    DBG_CONTINUE = 0x00010002
    DBG_EXCEPTION_NOT_HANDLED = 0x80010001
    DBG_EXCEPTION_HANDLED = 0x00010001


class ThreadInformationClass(enum.Enum):
...
class DebugExceptionCode(enum.Enum):
...
class ImageDirectory(enum.Enum):
...
class ThreadConstant(enum.Enum):
...
class ContextType(enum.Enum):
    ...
    CONTEXT_FULL = CONTEXT_x86 | CONTEXT_CONTROL | CONTEXT_INTEGER | CONTEXT_SEGMENTS
    CONTEXT_ALL = CONTEXT_x86 | CONTEXT_CONTROL | CONTEXT_INTEGER | CONTEXT_SEGMENTS | CONTEXT_FLOATING_POINT | \
...
class XSAVE_FORMAT_64(Structure):
    _fields_ = [
        ("ControlWord", WORD),
        ("StatusWord", WORD),
        ("TagWord", BYTE),
...
class XSAVE_FORMAT_32(Structure):
    _fields_ = [
        ("ControlWord", WORD),
        ("StatusWord", WORD),
        ("TagWord", BYTE),
...
class TMP_DUMMYSTRUCTNAME(Structure):
    _fields_ = [
        ("Header", M128A * 2),
        ("Legacy", M128A * 8),
        ("Xmm0", M128A),
...
class TMP_CONTEXT64_SUBUNION(Union):
    _fields_ = [
...
EXCEPTION_RECORD._fields_ = [
    ...
    ("ExceptionInformation", PULONG * EXCEPTION_MAXIMUM_PARAMETERS),
]
...
class TMP_UNION_DEBUG_INFO(Union):
    _fields_ = [
...
WINTRUST_ACTION_GENERIC_VERIFY_V2_STR = create_string_buffer(b"".join([struct.pack(i, j) for i, j in
                                                                       zip(IID_PACK,
                                                                           WINTRUST_ACTION_GENERIC_VERIFY_V2_RAW)]))
WINTRUST_ACTION_GENERIC_VERIFY_V2 = GUID.from_address(addressof(WINTRUST_ACTION_GENERIC_VERIFY_V2_STR))
...
PCERT_ENHKEY_USAGE = POINTER(CTL_USAGE)
...
PUNICODE_STRING = POINTER(LSA_UNICODE_STRING)
UNICODE_STRING = LSA_UNICODE_STRING
_LSA_UNICODE_STRING = LSA_UNICODE_STRING
