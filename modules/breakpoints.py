...
class SoftwareBreakpoint(__Breakpoint):
    def __init__(self, address, handler, original_byte):
        super(SoftwareBreakpoint).__init__(BreakPointType.SOFTWARE_BREAKPOINT, address, handler)
...
class MemoryBreakpoint(__Breakpoint):
    def __init__(self, address, handler, access, size):
        super(MemoryBreakpoint).__init__(BreakPointType.MEMORY_BREAKPOINT, address, handler)
...
class HardwareBreakpoint(__Breakpoint):
    def __init__(self, address, handler, access, size):
        super(HardwareBreakpoint).__init__(BreakPointType.HARDWARE_BREAKPOINT, address, handler)
        if access in [0, 1, 2]:
            self._access = access
        else:
