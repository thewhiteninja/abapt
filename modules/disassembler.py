        ...
        if file_exists(project.binary_path + ".cache"):
            ...
            for section in self._pe.sections:
                if section.Characteristics & SECTION_CHARACTERISTICS["IMAGE_SCN_MEM_EXECUTE"]:
                    ...
                    for i in self._disassembler.disasm(bytes(self._pe.helper.mapped[begin:]), begin):
    ...
    def __disass_block_from(self, begin, otherbranch=None):
        if begin in self._block_list:
            ...
            if begin in self._instructions_cache:
                ...
                rinsrt = list(self._disassembler.disasm(bytes(self._pe.helper.mapped[begin:begin + 16]), begin))
                if len(rinsrt) > 0:
                    i = rinsrt[0]
                else:
                    i = None
            while i is not None:
                if i.address != begin and i.address in self._block_list:
                    #log_debug("%08x 1-> %08x" % (
                        #.instructions[0].address, self._block_list[i.address].instructions[0].address))
                ...
                if i in self._instructions_cache:
                    ...
                    rinsrt = list(
                        self._disassembler.disasm(bytes(self._pe.helper.mapped[i.address + i.size:i.address + i.size + 10]),
                                                  i.address + i.size))
                    if len(rinsrt) > 0:
                        i = rinsrt[0]
                    else:
